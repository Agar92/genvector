...
#if defined(__sun)
// solaris definition of cmath does not include math.h which has the definitions
...
// The trick is to keep std::sqrt() dependent (on its argument type) by making
// it an unqualified name. The `std::` of `std::sqrt()` makes it a qualified
// name, so the code here has to use `sqrt()`, not `std::sqrt()`. To still
// find `std::sqrt()` we pull `std::sqrt()` into the surrounding namespace.
//
// We don't want to use 'using namespace std' because it would polute the
// including headers.
using std::acos;
using std::acosh;
using std::asin;
...
inline double log1p(double x)
{
  ...
  y = 1 + x;
  return std::log(y) -
         ((y - 1) - x) / y; /* cancels errors with IEEE arithmetic */
#endif
}
