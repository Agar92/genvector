...
// Header file for class Rotation in 3 dimensions, described by 3 Z-Y-X  Euler
// angles representing a rotation along Z, Y and X
//
...
/**
   ...
   Note that the rotations are extrinsic rotations happening around a fixed
   coordinate system. This is  different than the convention of the
   ROOT::Math::EulerAngles class, where the rotation are intrinsic. Also it has
   not to be confused with the typical Goldstein definition of the Euler Angles
   (Z-X-Z or 313 sequence) which is used by the ROOT::Math::EulerAngles class,
   while the sequence here is Z-Y-X or 321. Applying a RotationZYX(phi, theta,
   psi)  to a vector is then equal to applying RotationX(psi) * RotationY(theta)
...
public:
  ...
  */
  template <class IT> RotationZYX(IT begin, IT end)
  {
  ...
  */
  template <class OtherRotation> explicit RotationZYX(const OtherRotation &r);

  /**
     Assign from another supported rotation type (see gv_detail::convert )
  */
  template <class OtherRotation> RotationZYX &operator=(OtherRotation const &r);
  // ======== Components ==============
  ...
  */
  template <class R> RotationZYX &operator*=(const R &r)
  {
    return *this = (*this) * r;
  }
 ...
 */
template <class R>
inline typename RotationZYX::Scalar Distance(const RotationZYX &r1,
                                             const R &r2);
...
template <class CoordSystem, class U>
DisplacementVector3D<CoordSystem, U>
inline RotationZYX::operator()(const DisplacementVector3D<CoordSystem, U> &v) const
{
...
template <class CoordSystem>
LorentzVector<CoordSystem>
inline RotationZYX::operator()(const LorentzVector<CoordSystem> &v) const
{
...
inline RotationZYX::RotationZYX(const OtherRotation &r)
{
...
*/
template <class OtherRotation>
inline RotationZYX &RotationZYX::operator=(OtherRotation const &r)
{
  gv_detail::convert(r, *this);
  return *this;
}
...
/**
   Distance between two rotations
*/
template <class R> RotationZYX::Scalar RotationZYX::Distance(const R &r) const
{
