...
public:
  ...
  */
  template <class LAVector> PositionVector3D(const LAVector &v, size_t index0)
  {
  ...
  */
   ...
   */
  PositionVector3D<CoordSystem, Tag> &SetCoordinates(Scalar a, Scalar b,
                                                     Scalar c)
  {
   ...
   */
  template <class IT> void GetCoordinates(IT begin) const
  {
    Scalar a = Scalar(0);
    Scalar b = Scalar(0);
    Scalar c = Scalar(0);
          ...
          const PositionVector3D<CoordSystem2, U> &v2)
{
  return DisplacementVector3D<CoordSystem1, U>(
      Cartesian3D<typename CoordSystem1::Scalar>(
...
template <class char_t, class traits_t, class T, class U,
          typename std::enable_if<std::is_arithmetic<typename PositionVector3D<
              T, U>::Scalar>::value>::type * = nullptr>
