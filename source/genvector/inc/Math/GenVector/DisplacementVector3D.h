...
/**
   ...
   More information is available at the home page for \ref Vector
...
/**
 ...
 The class is having also an extra template parameter, the coordinate system
 tag, to be able to identify (tag) vector described in different reference
 coordinate system, like global or local coordinate systems.
...
public:
  ...
  */
  ...
  /**
       ...
       coordinates, or using a different Scalar type, but with same coordinate
     system tag
   */
  template <class OtherCoords>
  explicit constexpr DisplacementVector3D(
  ...
  */
  ...
  operator=(const DisplacementVector3D<OtherCoords, Tag> &v)
  {
    fCoordinates = v.Coordinates();
    return *this;
  }
  ...
  */
  template <class OtherCoords>
  constexpr DisplacementVector3D &
  operator=(const PositionVector3D<OtherCoords, Tag> &rhs)
  {
  ...
  */
  template <class ForeignVector>
  constexpr DisplacementVector3D &operator=(const ForeignVector &v)
  {
    SetXYZ(v.x(), v.y(), v.z());
    return *this;
  }
  ...
  */
  constexpr CoordSystem Coordinates() const { return fCoordinates; }

  /**
     Set internal data based on a C-style array of 3 Scalar numbers
   */
  constexpr DisplacementVector3D<CoordSystem, Tag> &
  SetCoordinates(const Scalar src[])
  {
  ...
  template <class IT>
#ifndef NDEBUG
  constexpr DisplacementVector3D<CoordSystem, Tag> &SetCoordinates(IT begin,
                                                                   IT end)
#else
   ...
   */
  constexpr void GetCoordinates(Scalar &a, Scalar &b, Scalar &c) const
  {
   ...
   */
  constexpr DisplacementVector3D<CoordSystem, Tag> &SetXYZ(Scalar a, Scalar b,
                                                           Scalar c)
  {
  ...
  /**
    Exact equality
   */
  constexpr bool operator==(const DisplacementVector3D &rhs) const
  {
  ...
  */
  constexpr Scalar X() const { return fCoordinates.X(); }

  /**
      Cartesian Y, converting if necessary from internal coordinate system.
  */
  constexpr Scalar Y() const { return fCoordinates.Y(); }

  /**
      Cartesian Z, converting if necessary from internal coordinate system.
  */
  constexpr Scalar Z() const { return fCoordinates.Z(); }
  ...
  */
  constexpr Scalar Mag2() const { return fCoordinates.Mag2(); }
  ...
  */
  template <typename SCALAR = Scalar,
            typename std::enable_if<std::is_arithmetic<SCALAR>::value>::type * =
                nullptr>
  constexpr DisplacementVector3D Unit() const
  {
    const auto tot = R();
  ...
  */
  constexpr DisplacementVector3D<CoordSystem, Tag> &SetX(Scalar xx)
  {
  ...
  */
  constexpr DisplacementVector3D<CoordSystem, Tag> &SetY(Scalar yy)
  {
  ...
  */
  constexpr DisplacementVector3D<CoordSystem, Tag> &SetTheta(Scalar ang)
  {
  ...
  */
  constexpr DisplacementVector3D<CoordSystem, Tag> &SetPhi(Scalar ang)
  {
  ...
  */
  constexpr DisplacementVector3D<CoordSystem, Tag> &SetRho(Scalar rr)
  {
  ...
  */
  constexpr DisplacementVector3D<CoordSystem, Tag> &SetEta(Scalar etaval)
  {
  ...
  */
  template <class OtherCoords>
  constexpr Scalar Dot(const DisplacementVector3D<OtherCoords, Tag> &v) const
  {
  ...
  */
  template <class OtherVector> constexpr Scalar Dot(const OtherVector &v) const
  {
    return X() * v.x() + Y() * v.y() + Z() * v.z();
  }
  ...
  */
  template <class OtherCoords>
  constexpr DisplacementVector3D
  Cross(const DisplacementVector3D<OtherCoords, Tag> &v) const
  {
    DisplacementVector3D result;
    result.SetXYZ(Y() * v.Z() - v.Y() * Z(), Z() * v.X() - v.Z() * X(),
                  X() * v.Y() - v.X() * Y());
    return result;
  }
  /**
  ...
  */
  ...
  constexpr DisplacementVector3D Cross(const OtherVector &v) const
  {
    DisplacementVector3D result;
    result.SetXYZ(Y() * v.z() - v.y() * Z(), Z() * v.x() - v.z() * X(),
                  X() * v.y() - v.x() * Y());
  ...
  */
  constexpr DisplacementVector3D &operator*=(Scalar a)
  {
  ...
  */
  constexpr DisplacementVector3D operator*(Scalar a) const
  {
   ...
   */
  constexpr DisplacementVector3D operator-() const
  {
   ...
   */
  constexpr DisplacementVector3D operator+() const { return *this; }
  ...
  constexpr Scalar mag2() const { return fCoordinates.Mag2(); }
  constexpr Scalar perp2() const { return fCoordinates.Perp2(); }
  constexpr DisplacementVector3D unit() const { return Unit(); }
...
private:
  ...
  */
  ...
  template <class OtherCoords, class OtherTag>
  constexpr DisplacementVector3D &
  operator=(const DisplacementVector3D<OtherCoords, OtherTag> &);

  template <class OtherCoords, class OtherTag>
  constexpr DisplacementVector3D &
  operator=(const PositionVector3D<OtherCoords, OtherTag> &);
  ...
  template <class OtherCoords, class OtherTag>
  constexpr Scalar
  Dot(const DisplacementVector3D<OtherCoords, OtherTag> &) const;

  template <class OtherCoords, class OtherTag>
  constexpr DisplacementVector3D
  Cross(const DisplacementVector3D<OtherCoords, OtherTag> &) const;
};
...
inline constexpr DisplacementVector3D<CoordSystem1, U>
...
inline constexpr DisplacementVector3D<CoordSystem, U>
...
template <
    ...
    typename std::enable_if<std::is_arithmetic<
        typename DisplacementVector3D<T, U>::Scalar>::value>::type * = nullptr>
