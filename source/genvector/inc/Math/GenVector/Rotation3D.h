...
/**
   ...
   See also ROOT::Math::AxisAngle, ROOT::Math::EulerAngles, and
   ROOT::Math::Quaternion for classes which have conversion operators to
   Rotation3D.

   All Rotations types (not only Rotation3D) can be applied to all 3D Vector
   classes (like ROOT::Math::DisplacementVector3D and
   ROOT::Math::PositionVector3D) and also to the 4D Vectors
   (ROOT::Math::LorentzVector classes), acting on the 3D components. A rotation
   operation is applied by using the operator() or the operator *. With the
   operator * is possible also to combine rotations. Note that the operator is
   NOT commutative, the order how the rotations are applied is relevant.
...
public:
  ...
  */
  template <class IT> Rotation3D(IT begin, IT end)
  {
  ...
  */
  template <class ForeignMatrix> explicit Rotation3D(const ForeignMatrix &m)
  {
    SetComponents(m);
  }
  ...
  */
  ...
  Rotation3D(const ForeignVector &v1, const ForeignVector &v2,
  ...
  */
  Rotation3D(Scalar xx, Scalar xy, Scalar xz, Scalar yx, Scalar yy, Scalar yz,
  ...
  */
  ...
  {
    SetComponents(rhs.fM[0], rhs.fM[1], rhs.fM[2], rhs.fM[3], rhs.fM[4],
                  rhs.fM[5], rhs.fM[6], rhs.fM[7], rhs.fM[8]);
    return *this;
  }
  ...
  */
  ...
  /**
     ...
     which must support operator()(i,j) to obtain elements (0,0) thru (2,2).
  */
  template <class ForeignMatrix> Rotation3D &operator=(const ForeignMatrix &m)
  {
    SetComponents(m(0, 0), m(0, 1), m(0, 2), m(1, 0), m(1, 1), m(1, 2), m(2, 0),
                  m(2, 1), m(2, 2));
    return *this;
  }
  ...
  */
  template <class ForeignVector>
  void SetComponents(const ForeignVector &v1, const ForeignVector &v2,
                     const ForeignVector &v3)
  {
    fM[kXX] = v1.x();
    fM[kXY] = v2.x();
  ...
  */
  template <class ForeignVector>
  void GetComponents(ForeignVector &v1, ForeignVector &v2,
  ...
  */
  void SetComponents(Scalar xx, Scalar xy, Scalar xz, Scalar yx, Scalar yy,
                     Scalar yz, Scalar zx, Scalar zy, Scalar zz)
  {
    fM[kXX] = xx;
    fM[kXY] = xy;
    fM[kXZ] = xz;
  ...
  */
  void GetComponents(Scalar &xx, Scalar &xy, Scalar &xz, Scalar &yx, Scalar &yy,
                     Scalar &yz, Scalar &zx, Scalar &zy, Scalar &zz) const
  {
    xx = fM[kXX];
    xy = fM[kXY];
    xz = fM[kXZ];
  ...
  */
   ...
   */
  // note: cannot have a  template method since it is ambigous with the operator
  // * on vectors
  ...
  */
  template <class R> Rotation3D &operator*=(const R &r)
  {
    return *this = (*this) * r;
  }
...
private:
  std::array<Scalar, 9> fM; // 9 elements (3x3 matrix) representing the rotation
...
template <class CoordSystem>
inline LorentzVector<CoordSystem> Rotation3D::
operator()(const LorentzVector<CoordSystem> &v) const
{
...
*/
template <class ForeignVector>
inline ForeignVector Rotation3D::operator()(const ForeignVector &v) const
{
...
/**
   Construct from an AxisAngle
*/
inline Rotation3D::Rotation3D(AxisAngle const &a)
{
...
/**
   Construct from EulerAngles
*/
inline Rotation3D::Rotation3D(EulerAngles const &e)
{
...
/**
   Construct from RotationZYX
*/
inline Rotation3D::Rotation3D(RotationZYX const &e)
{
  gv_detail::convert(e, *this);
}
...
/**
   Construct from a Quaternion
*/
inline Rotation3D::Rotation3D(Quaternion const &q)
{
...
/**
   Construct from an axial rotation
*/
inline Rotation3D::Rotation3D(RotationZ const &r)
{
  gv_detail::convert(r, *this);
}
inline Rotation3D::Rotation3D(RotationY const &r)
{
  gv_detail::convert(r, *this);
}
inline Rotation3D::Rotation3D(RotationX const &r)
{
  gv_detail::convert(r, *this);
}
...
inline void Rotation3D::Rectify()
{
  ...
  double u21 = m12 / u11;
  double u31 = m13 / u11;
  double u22 = sqrt(m22 - u21 * u21);
  double u32 = (m23 - m12 * m13 / m11) / u22;
  double u33 = sqrt(m33 - u31 * u31 - u32 * u32);
...
inline void Rotation3D::Invert()
{
  // invert a rotation
  std::swap(fM[kXY], fM[kYX]);
  std::swap(fM[kXZ], fM[kZX]);
  std::swap(fM[kYZ], fM[kZY]);
}

inline Rotation3D Rotation3D::operator*(const AxisAngle &a) const
{
  // combine with an AxisAngle rotation
  return operator*(Rotation3D(a));
}

inline Rotation3D Rotation3D::operator*(const EulerAngles &e) const
{
  // combine with an EulerAngles rotation
  return operator*(Rotation3D(e));
}

inline Rotation3D Rotation3D::operator*(const Quaternion &q) const
{
  // combine with a Quaternion rotation
  return operator*(Rotation3D(q));
}

inline Rotation3D Rotation3D::operator*(const RotationZYX &r) const
{
