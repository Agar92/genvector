...
template <class R1, class R2> void convert(R1 const &, R2 const)
{
  ERROR_This_Rotation_Conversion_is_NOT_Supported();
}
...
// ----------------------------------------------------------------------
inline void convert(Rotation3D const &from, AxisAngle &to)
{
  // conversions from Rotation3D
  double m[9];
...
inline static void correctByPi(double &psi, double &phi)
{
  static const double pi = M_PI;
  if (psi > 0)
  {
...
inline void convert(Rotation3D const &from, EulerAngles &to)
{
  ...
  theta =
      (std::fabs(r[kZZ]) <= 1.0) ? std::acos(r[kZZ]) : (r[kZZ] > 0.0) ? 0 : pi;
  ...
  case 1:
    ...
    break;
  case 2:
    if (w[2] > 0 && std::fabs(psi) > pi_2)
      correctByPi(psi, phi);
    if (w[2] < 0 && std::fabs(psi) < pi_2)
      correctByPi(psi, phi);
    break;
  case 3:
    if (w[3] > 0 && std::fabs(phi) > pi_2)
      correctByPi(psi, phi);
    if (w[3] < 0 && std::fabs(phi) < pi_2)
      correctByPi(psi, phi);
    break;
...
inline void convert(Rotation3D const &from, Quaternion &to)
{
  double m[9];
  from.GetComponents(m, m + 9);
...
/// same Math used as for EulerAngles apart from some different meaning of
/// angles and matrix elements. But the basic algoprithms principles are the
/// same described in
/// http://www.cern.ch/mathlibs/documents/eulerAngleComputation.pdf

inline void convert(Rotation3D const &from, RotationZYX &to)
{
  ...
  // since we determine phi+psi or phi-psi phi and psi can be both have a shift
  // of +/- PI.
  // The shift must be applied on both (the sum (or difference) is knows to +/-
  // 2PI )
  // This can be fixed looking at the other 4 matrix terms, which have terms in
  // sin and cos of psi
  // and phi. sin(psi+/-PI) = -sin(psi) and cos(psi+/-PI) = -cos(psi).
  // Use then the biggest term for making the correction to minimize possible
  // numerical errors
...
inline void convert(AxisAngle const &from, Rotation3D &to)
{
...
inline void convert(AxisAngle const &from, EulerAngles &to)
{
...
inline void convert(AxisAngle const &from, Quaternion &to)
{
  ...
  to.SetComponents(std::cos(from.Angle() / 2), s * axis.X(), s * axis.Y(),
                   s * axis.Z());
} // convert to Quaternion

inline void convert(AxisAngle const &from, RotationZYX &to)
{
...
inline void convert(EulerAngles const &from, Rotation3D &to)
{
  ...
  to.SetComponents(cPsi * cPhi - sPsi * cTheta * sPhi,
                   cPsi * sPhi + sPsi * cTheta * cPhi, sPsi * sTheta,
                   -sPsi * cPhi - cPsi * cTheta * sPhi,
                   -sPsi * sPhi + cPsi * cTheta * cPhi, cPsi * sTheta,
                   sTheta * sPhi, -sTheta * cPhi, cTheta);
}

inline void convert(EulerAngles const &from, AxisAngle &to)
{
...
inline void convert(EulerAngles const &from, Quaternion &to)
{
  ...
  to.SetComponents(cTheta * cPlus, -sTheta * cMinus, -sTheta * sMinus,
                   -cTheta * sPlus);
  // TODO -- carefully check that this is correct
}

inline void convert(EulerAngles const &from, RotationZYX &to)
{
...
inline void convert(Quaternion const &from, Rotation3D &to)
{
  ...
  to.SetComponents(q00 + q11 - q22 - q33, 2 * (q12 - q03), 2 * (q02 + q13),
                   2 * (q12 + q03), q00 - q11 + q22 - q33, 2 * (q23 - q01),
                   2 * (q13 - q02), 2 * (q23 + q01), q00 - q11 - q22 + q33);

} // conversion to Rotation3D

inline void convert(Quaternion const &from, AxisAngle &to)
{
  ...
  if (u >= 0)
  {
    ...
    DisplacementVector3D<Cartesian3D<double>> axis(from.I(), from.J(),
                                                   from.K());
...
inline void convert(Quaternion const &from, EulerAngles &to)
{
...
inline void convert(Quaternion const &from, RotationZYX &to)
{
...
// ----------------------------------------------------------------------
// conversions from RotationZYX
inline void convert(RotationZYX const &from, Rotation3D &to)
{
  ...
  to.SetComponents(std::cos(theta) * std::cos(phi),
                   ...
                   std::cos(psi) * std::sin(phi) +
                       std::sin(psi) * std::sin(theta) * std::cos(phi),
                   std::cos(psi) * std::cos(phi) -
                       std::sin(psi) * std::sin(theta) * std::sin(phi),
                   -std::sin(psi) * std::cos(theta),

                   std::sin(psi) * std::sin(phi) -
                       std::cos(psi) * std::sin(theta) * std::cos(phi),
                   std::sin(psi) * std::cos(phi) +
                       std::cos(psi) * std::sin(theta) * std::sin(phi),
                   std::cos(psi) * std::cos(theta));
}
inline void convert(RotationZYX const &from, AxisAngle &to)
{
...
inline void convert(RotationZYX const &from, EulerAngles &to)
{
...
inline void convert(RotationZYX const &from, Quaternion &to)
{
  double phi, theta, psi = 0;
  from.GetComponents(phi, theta, psi);
...
inline void convert(RotationX const &from, Rotation3D &to)
{
...
inline void convert(RotationX const &from, AxisAngle &to)
{
...
inline void convert(RotationX const &from, EulerAngles &to)
{
...
inline void convert(RotationX const &from, Quaternion &to)
{
  // conversion from RotationX to Quaternion

  to.SetComponents(std::cos(from.Angle() / 2), std::sin(from.Angle() / 2), 0,
                   0);
}

inline void convert(RotationX const &from, RotationZYX &to)
{
...
inline void convert(RotationY const &from, Rotation3D &to)
{
...
inline void convert(RotationY const &from, AxisAngle &to)
{
...
inline void convert(RotationY const &from, EulerAngles &to)
{
...
inline void convert(RotationY const &from, RotationZYX &to)
{
  // conversion from RotationY to RotationZYX
  to.SetComponents(0, from.Angle(), 0);
}

inline void convert(RotationY const &from, Quaternion &to)
{
  // conversion from RotationY to Quaternion

  to.SetComponents(std::cos(from.Angle() / 2), 0, std::sin(from.Angle() / 2),
                   0);
}
...
inline void convert(RotationZ const &from, Rotation3D &to)
{
...
inline void convert(RotationZ const &from, AxisAngle &to)
{
...
inline void convert(RotationZ const &from, EulerAngles &to)
{
...
inline void convert(RotationZ const &from, RotationZYX &to)
{
  // conversion from RotationY to RotationZYX
  to.SetComponents(from.Angle(), 0, 0);
}

inline void convert(RotationZ const &from, Quaternion &to)
{
  // conversion from RotationZ to Quaternion

  to.SetComponents(std::cos(from.Angle() / 2), 0, 0,
                   std::sin(from.Angle() / 2));
}
