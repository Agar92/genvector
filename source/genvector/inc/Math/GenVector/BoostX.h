...
inline BoostX::BoostX() : fBeta(0.0), fGamma(1.0) {}

inline void BoostX::SetComponents(Scalar bx) {
  // set component
  Scalar bp2 = bx * bx;
  if (bp2 >= 1) {
    GenVector::Throw(
...
inline void BoostX::GetComponents(Scalar &bx) const {
  // get component
  bx = fBeta;
}

inline DisplacementVector3D<Cartesian3D<BoostX::Scalar>>
BoostX::BetaVector() const {
  // return beta vector
  return DisplacementVector3D<Cartesian3D<Scalar>>(fBeta, 0.0, 0.0);
}

inline void BoostX::GetLorentzRotation(Scalar r[]) const {
...
inline LorentzVector<PxPyPzE4D<double>> BoostX::
operator()(const LorentzVector<PxPyPzE4D<double>> &v) const {
  // apply boost to a LV
  Scalar x = v.Px();
  Scalar t = v.E();
  return LorentzVector<PxPyPzE4D<double>>(fGamma * x + fGamma * fBeta * t,
                                          v.Py(), v.Pz(),
                                          fGamma * fBeta * x + fGamma * t);
}

inline void BoostX::Invert() {
  // invert
  fBeta = -fBeta;
}

inline BoostX BoostX::Inverse() const {
