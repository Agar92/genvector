...
/**
   Rotation class representing a 3D rotation about the Z axis by the angle of
   rotation. For efficiency reason, in addition to the the angle, the sine and
   cosine of the angle are held
...
public:
  ...
  */
  explicit RotationZ(Scalar angle)
      : fAngle(angle), fSin(std::sin(angle)), fCos(std::cos(angle))
  {
    Rectify();
  }
